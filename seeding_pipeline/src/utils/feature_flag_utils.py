...
def export_feature_flags(output_path: Optional[Path] = None, format: str = "json") -> Dict[str, Any]:
    """
...
def validate_feature_flags() -> bool:
    ...
    if (manager.is_enabled(FeatureFlag.SCHEMALESS_MIGRATION_MODE) and 
        not manager.is_enabled(FeatureFlag.ENABLE_SCHEMALESS_EXTRACTION)):
        issues.append(
    ...
    if (manager.is_enabled(FeatureFlag.ENABLE_ENTITY_RESOLUTION_V2) and
        not manager.is_enabled(FeatureFlag.ENABLE_SCHEMALESS_EXTRACTION)):
        issues.append(
