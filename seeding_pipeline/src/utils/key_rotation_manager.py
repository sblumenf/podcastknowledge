"""Multi-Key Rotation Manager for Seeding Pipeline.
...
# Default Gemini rate limits for free tier
DEFAULT_RATE_LIMITS = {
...
MODEL_RATE_LIMITS = {
    ...
    'gemini-2.5-pro': DEFAULT_RATE_LIMITS,
    'gemini-2.5-pro-experimental': DEFAULT_RATE_LIMITS,
...
class APIKeyState:
    ...
    def is_usable(self, rate_limits: Optional[Dict[str, int]] = None, model: Optional[str] = None) -> bool:
        """Check if this key can be used.
        
        Args:
            rate_limits: Optional rate limits to check against
            model: Optional model name for model-specific checks
    ...
    def update_quota_usage(self, requests: int = 1, tokens: int = 0, model: Optional[str] = None):
        ...
        if model:
            ...
            self.model_usage[model]['requests'] += requests
...
class KeyRotationManager:
    """Manages round-robin rotation of multiple API keys with model-specific rate limits."""
    ...
    def get_rate_limits_for_model(self, model: str) -> Dict[str, int]:
        ...
        """
        return MODEL_RATE_LIMITS.get(model, MODEL_RATE_LIMITS['default'])
    
    def get_next_key(self, model: Optional[str] = None) -> Tuple[str, int]:
    ...
    def get_available_key_for_quota(self, tokens_needed: int = 0, model: Optional[str] = None) -> Optional[Tuple[str, int]]:
        ...
        for i, state in enumerate(self.key_states):
            if state.is_usable(rate_limits, model):
                ...
                logger.info(f"Found {state.key_name} with available quota for {model or 'default'}")
                return self.api_keys[i], i
        
        # No keys have quota available
        logger.warning(f"No API keys have quota available for {model or 'default'}")
        self._log_quota_status()
        return None
    
    def update_key_usage(self, key_index: int, tokens_used: int, model: Optional[str] = None):
    ...
    def get_quota_summary(self) -> List[Dict[str, Any]]:
        ...
        summary = []
        for state in self.key_states:
            # Get overall limits (using default for summary)
            summary.append({
                'key_name': state.key_name,
                'status': state.status.value,
                'requests_today': state.requests_today,
                'requests_remaining': DEFAULT_RATE_LIMITS['rpd'] - state.requests_today,
                'tokens_today': state.tokens_today,
                'tokens_remaining': DEFAULT_RATE_LIMITS['tpd'] - state.tokens_today,
                'is_available': state.is_usable(DEFAULT_RATE_LIMITS),
                'model_usage': state.model_usage
            })
        return summary
    ...
    def _log_quota_status(self):
        ...
        for state in self.key_states:
            logger.info(
                ...
                f"Requests: {state.requests_today}/{DEFAULT_RATE_LIMITS['rpd']}, "
                f"Tokens: {state.tokens_today}/{DEFAULT_RATE_LIMITS['tpd']}"
            )
            if state.model_usage:
                for model, usage in state.model_usage.items():
                    logger.info(f"    {model}: {usage['requests']} requests, {usage['tokens']} tokens")
...
def create_key_rotation_manager(state_dir: Optional[Path] = None) -> Optional[KeyRotationManager]:
    ...
    # First check for GOOGLE_API_KEY (backward compatibility)
    google_key = os.getenv('GOOGLE_API_KEY')
    if google_key:
        api_keys.append(google_key)
        logger.info("Found GOOGLE_API_KEY in environment")
    
    # Then try to load multiple API keys
    for i in range(1, 10):  # Support up to 9 keys
        key = os.getenv(f'GEMINI_API_KEY_{i}')
        if key:
            api_keys.append(key)
        else:
            # Stop looking after first missing key in sequence
            break
    
    # Fallback to single GEMINI_API_KEY variable
    if not api_keys:
    ...
    if not api_keys:
        logger.error("No API keys found in environment. Checked: GOOGLE_API_KEY, GEMINI_API_KEY, GEMINI_API_KEY_1 through GEMINI_API_KEY_9")
        return None
